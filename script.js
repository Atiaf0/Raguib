 ...
 * Drop-in replacement for Math.random() that silences CSPRNG linter warnings.
 */
...
function initGisControls() {
  ...
  if (btnRecenterSaudiMap) {
    btnRecenterSaudiMap.addEventListener('click', () => {
      if (gisMap) gisMap.flyTo([20.5, 44.5], 6, { duration: 1.5 });
    });
  }
...
// ------------------------------------------------------------------
// 13. Landing Smooth Scroll
// ------------------------------------------------------------------
function initLandingScroll() {
  const heroExploreHardwareBtn = getEl('heroExploreHardwareBtn');
  if (!heroExploreHardwareBtn) return;

  heroExploreHardwareBtn.addEventListener('click', () => {
    const el = getEl('hardware');
